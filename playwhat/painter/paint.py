...
def paint_recently_played(options: RecentTrackOptions) -> Image.Image:
    """
    Paints the "Recently Played" screen and returns a `PIL.Image` that can be shown on the InkyWHAT
    display
    """
    image = Image.new("P", (InkyWHAT.WIDTH, InkyWHAT.HEIGHT))
    image.putpalette((
    ...
    draw = ImageDraw.Draw(image)
    _paint_recently_played_header(image, draw)
    _paint_recently_played_list(image, draw, options)
    _paint_recently_played_footer(image, draw, options)
...
def _paint_recently_played_header(image: Image.Image, draw: ImageDraw.ImageDraw):
     # Draw the Spotify logo
    with Image.open(os.path.join(PATH_ASSET_IMAGE, "logo-spotify.png")) as logo:
        logo_height = logo.height
        logo_width = logo.width
...
def _paint_recently_played_list(
    image: Image.Image,
...
def _paint_recently_played_footer(
    image: Image.Image,
    draw: ImageDraw.ImageDraw,
    options: RecentTrackOptions):
    # Draw the user's icon
